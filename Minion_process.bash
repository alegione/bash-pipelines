...
#echo -e "${RED}DEBUG: Sleeping to give you time to check on the last loop (porechop)${NOCOLOUR}"
#sleep 5m

# COLLECT NAMES OF FILES FROM PORECHOP OUTPUT
if [ ! -e "$Meta/ReadFileNames.txt" ]; then
  echo -e "${PURPLE}$(date)${NOCOLOUR}" | tee -a $Progress
  echo -e "${BLUE}Producing list of read files${NOCOLOUR}" | tee -a $Progress
  if [ $Barcoded == "TRUE" ]; then
    echo -e "${RED}DEBUG: making list for barcoded sequences${NOCOLOUR}"
    basename -a -s ".fastq" $DIR_TrimmedReads/* > "$Meta/ReadFileNames.txt"
    while read i; do
      echo -e "${RED}DEBUG: In moving loop for $i${NOCOLOUR}"
      if [ ! -e "$DIR_TrimmedReads/$i.porechop.fastq" ]; then
              echo -e "${RED}DEBUG: In conditional move inside loop for $i${NOCOLOUR}"
        mv "$DIR_TrimmedReads/$i.fastq" "$DIR_TrimmedReads/$i.porechop.fastq"
      fi
    done < "$Meta/ReadFileNames.txt"
  else
    echo -e "${RED}DEBUG: Making a list for non-barcoded reads $i${NOCOLOUR}"
    basename -a -s ".porechop.fastq" $DIR_TrimmedReads/* > "$Meta/ReadFileNames.txt"
  fi
fi


#echo -e "${RED}DEBUG: Sleeping to give you time to check on the last loop (filename list)${NOCOLOUR}"
#sleep 5m
...
if [ ! -e "$DIR_ResultsSum/Stats-02-PoreChop_reads.txt" ]; then
  ...
  while read i; do
#      if [ $Direction != "1D2" ]; then
    echo -e "\n======================\n$i.porechop.fastq\n" >> "$DIR_ResultsSum/Stats-02-PoreChop_reads.txt"
...
if [ ! -e "$DIR_ResultsSum/Stats-03-FiltLong_reads.txt" ]; then
	...
	NanoStat --version | tee -a $Progress
	while read i; do
		echo -e "\n======================\n$i.filtered.fastq\n" >> "$DIR_ResultsSum/Stats-03-FiltLong_reads.txt"
...
echo "debug: The reference genome variable is set to $RefGenome"
...
#IF USED DIRECTED RNA PROTOCOL, CONVERT RNA TO DNA for ASSEMBLY
if [ "$Product" == "RNA" ]; then
